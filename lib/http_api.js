...
class http_api {
	...
	getRobotModel(duid) {
		if (!duid) {
			throw new Error("Parameter duid missing on function getRobotModel");
		}

		const devices = this.getDevices();
		const products = this.getProducts();

		const productDevice = devices.find((device) => device.duid === duid);

		if (productDevice) {
			const productID = productDevice.productId;
